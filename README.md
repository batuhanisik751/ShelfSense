...
- **Phase 1.4 – pantry page**: [src/app/(app)/pantry/page.tsx](src/app/(app)/pantry/page.tsx) is now a real async server component that RLS-selects `pantry_items` for the logged-in user, groups by `category ?? 'other'`, sorts groups alphabetically with **Other** pinned last, and renders a per-group heading above a `PantryList` grid of `PantryItemCard`s (name + `qty unit · purchase date`). Belt-and-braces auth gate: `auth.getUser()` redirects to `/login?redirectTo=/pantry` even though middleware already blocks unauthed access. A new pure helper [src/lib/pantry/categoryLabels.ts](src/lib/pantry/categoryLabels.ts) maps snake_case keys (`leafy_greens`) to Title Case labels (`Leafy Greens`), collapsing `null` and `'other'` to `Other`. The empty state is a dashed-border card with a primary **Upload a receipt** button (plain `<a>` + `buttonVariants()` to dodge App Router prefetch staleness). Per-card category badge was dropped post-audit because the group heading already shows the label. Query errors `throw` to the Next error boundary; zero `'use client'` directives; `rls-security-auditor` clean (0 must-fix). Phase 2 columns (`estimated_expiration_at`, `status`) are selected into the query but not yet rendered — Phase 2.5 will light them up without touching this page. Closes the Phase 1 loop: upload → OCR → Groq → pantry is now a visible end-to-end flow. `/receipts` list page (Phase 1.5) is still the placeholder.
